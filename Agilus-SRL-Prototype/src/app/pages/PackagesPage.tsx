...
export function PackagesPage() {
  ...
  const tabs = ['Full Body', 'Diabetes', 'Thyroid', "Women's Care", "Men's Care", 'Senior Care'];

  const currentPackages = packages.filter((pkg: any) => {
    if (activeTab === "Women's Care") {
  ...
  return (
    <div className="bg-[#f7f9fa] min-h-screen pt-8 pb-20 font-sans">
      <div className="max-w-[1280px] mx-auto px-6">
        ...
        {/* Categories Section */}
        <div className="text-center lg:text-center text-left mb-4 lg:mb-6">
        ...
        <h1 className="text-[16px] font-semibold text-gray-900 mb-6">Showing {currentPackages.length} tests/checkups</h1>
        
        {/* Changed from xl:grid-cols-4 to lg:grid-cols-3 to keep cards wider (landscape) */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {currentPackages.map((pkg: any, idx: number) => {
            ...
            return (
              <article key={idx} className="bg-white border border-gray-100 rounded-2xl lg:rounded-2xl shadow-sm relative h-full flex flex-col hover:shadow-[0_12px_40px_rgba(0,0,0,0.06)] transition-all duration-300">
                
                {pkg.popular && (
                  <img src={discountPill} alt="Discount" className="absolute top-[-25px] right-[-20px] w-16 h-auto z-20 drop-shadow-md" />
                )}
                ...
                <div className="bg-gradient-to-r from-[#0C1E40] to-[#16458B] p-4 lg:p-6 text-white flex justify-between items-start h-[80px] lg:h-[110px] rounded-t-2xl">
                  <h3 className="font-bold text-[14px] max-w-[65%] leading-[1.2] tracking-tight">{pkg.name}</h3>
                  <div className="text-right whitespace-nowrap">
                    <div className="text-[11px] text-blue-200/70 line-through mb-0.5">₹ {pkg.originalPrice}</div>
                    <div className="font-bold text-[18px] leading-none">₹ {pkg.price}</div>
                  </div>
                </div>
                ...
                <div className="p-3 lg:p-6 flex-1 flex flex-col justify-between bg-[#F8FAFC] rounded-b-2xl">
                  <div className="flex items-start gap-2 lg:gap-4 mb-3 lg:mb-6">
                    ...
                    <div className="flex items-start gap-1.5 lg:gap-2.5 flex-1 bg-white p-2 lg:p-3.5 rounded-xl border border-gray-50 shadow-sm">
                      ...
                      <span className="text-[10px] lg:text-[12px] text-gray-700 leading-tight font-medium">Reports within<br/>{pkg.reports}</span>
                    </div>
                  </div>
                  <div className="flex items-center gap-2 lg:gap-3 w-full mt-auto">
                    <button onClick={() => (window as any).appNavigate('/package/' + pkg.id)} className="flex-1 py-1.5 lg:py-3 border border-[#0076BC] text-[#0076BC] rounded-md lg:rounded-xl font-bold text-[10px] lg:text-[14px] hover:bg-blue-50 transition-colors bg-white">View Details</button>
                    <button 
